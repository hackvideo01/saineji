<?php
	...
	function change_status($ID){
		...
		$sql =  "DELETE FROM `nightlife` WHERE  ID = ".$ID;

		$rec = $db->execute($sql, $params);
		// if($rec==1)
		// {

			$sql = "SELECT * FROM `upload_file` WHERE 1 and Type =4 and ID_Mater = ".$ID;
			...
			$sql =  "DELETE FROM `upload_file` WHERE  1 and Type =4 and ID_Mater = ".$ID;
			$rec = $db->execute($sql, $params);

			$sql =  "DELETE FROM `user` WHERE 1 and role=5 and  ID_Mater = ".$ID;
			$rec = $db->execute($sql, $params);
		// }
		return;
	}
