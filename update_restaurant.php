<?php
	...
	function change_status($ID){
		...
		// {
			...
			$sql = "SELECT * FROM `upload_file` WHERE  1  and Type=1 and ID_Mater=".$ID;
			...
			$sql =  "DELETE FROM `upload_file` WHERE  1  and Type=1 and ID_Mater=".$ID;
			$rec = $db->execute($sql, $params);

			$sql =  "DELETE FROM `user` WHERE 1 and role=2 and  ID_Mater = ".$ID;
			$rec = $db->execute($sql, $params);
		// }
		return;
	}
