<?php
	...
	**/
	...
	/**
		* メインネーム
	**/
	$main_name = "nightlife_management";
	$tbl_name = "nightlife";
	...
	*∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞∞*/
	function get_list() 
	{
		...
		foreach ($rec as $key => $value) {
			$sql_img = "select * from upload_file where Type = 6 and ID_Mater = ".$value["ID"];
			$res_img = $db->get_one($sql_img,$params);
			if($res_img !=null){
				$rec[$key]["UploadPath"] = $res_img["UploadPath"];
			}else{
				$sql_img = "select * from upload_file where Type = 4 and ID_Mater = ".$value["ID"];
				$res_img = $db->get_one($sql_img,$params);
				if($res_img !=null){
					$rec[$key]["UploadPath"] = $res_img["UploadPath"];
				}
