<?php
	...
	**/
	...
	function eatCategory(){
		...
		$sql = "SELECT * FROM `category_nightlife` WHERE 1";
	...
	function postdata(){
		...
		if ($_REQUEST["ID"]=="nightlife"){
			...
			$sqlCategory = "SELECT * FROM `category_nightlife` WHERE id=".$_REQUEST['Category_id']." ";

			$data_category = $db->get_one($sqlCategory,$params);

			$categoryName = $data_category["Name"];

			$sql = "INSERT INTO `nightlife`( `Category_id`, `Category`, `Name`, `Introduce_cm`, `Address`, `Time_work`, `MemberEmail`, `UserName`, `Password`, `Tel`, `Text1`, `Text2`, `Text3`, `Text4`, `Text5`,   `QR_1`, `QR_2`, `Lat`, `Lng`,  `Flag`,FromDay,ToDay, `AdFlag`,`UpdatedBy`, `CreatedBy`,`UpdatedDate`,  `CreatedDate`)
				 VALUES  ('"
			...
			$sql = "INSERT INTO `user`(`ID_Mater`, `username`, `password`,Email, `role`, `role_name`, `comment`, `createddate`) 
				VALUES ($lastID,'".$username."','".$password."','".$_REQUEST['MemberEmail']."',5,'夜遊びのユーザ','".$_REQUEST['Name']."',now())";
			...
			if(count($data_image)>0 && $_REQUEST["ID"] >0)
 			...
 			// get user data
 			$sql = "SELECT * FROM `user` where role=5 and ID_Mater=".$_REQUEST["ID"];
			$rec_user = $db->get_one($sql,$params);
 			// if($_SESSION['role'] ==1){
				$subject= "夜遊びの情報を更新しました。";
 				$content= $_REQUEST['Name']."　様<br><br>";
	 			// $content .= date("Y")."年".date("m")."月".date("d")."日  ".date("H")+'7'."時".date("i")."分 <br>";
	 			$content.= "夜遊びの情報が更新されました。<br><br>";
	 			$content.= "【ログイン情報】※<br>";
	 			$content.= "サイト：".WEB_URL."<br>";
	 			if($rec_user!=null){
	...
	function getdataByID(){
		...
		global $vw;
		$params =[];
		$sql = "SELECT * FROM `nightlife` where ID=".$_REQUEST["ID"];
		$data_post = $db->get_one($sql,$params);
		$sql = "SELECT * FROM `upload_file` WHERE 1 and Type = 4 and ID_Mater=".$_REQUEST["ID"];
		...
		// get image banner
		$sql = "SELECT * FROM `upload_file` WHERE 1 and Type = 6 and ID_Mater=".$_REQUEST["ID"];
